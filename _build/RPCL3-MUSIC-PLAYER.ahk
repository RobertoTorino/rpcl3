...
logFile                 := A_ScriptDir  . "\rpcl3_music_player.log"
iniFile                 := A_ScriptDir  . "\rpcl3_music_player.ini"
global LastMetaFile := ""
...
Gui, +AlwaysOnTop
Gui, +LastFound
Gui, Add, ListView, vTrackList gTrackClicked BackgroundTrans cCCCCCC x0 y0 w515 h227 AltSubmit Grid, Name|Size|Bitrate|Type

Gui, Add, Progress, x0 y0 w515 h1 0x10 BackgroundFFCC66 Disabled
...
Gui, Add, Progress, x0 y227 w515 h1 0x10 BackgroundFFCC66 Disabled
Gui, Add, Text, gPlayPause                x0 y228 w100 h23 BackgroundTrans cCCCCCC Border +Center +0x200, Play/Pause
Gui, Add, Text, gStopTrack              x100 y228 w75 h23 BackgroundTrans cCCCCCC Border +Center +0x200, Stop
Gui, Add, Text, gNextTrack              x175 y228 w75 h23 BackgroundTrans cCCCCCC Border +Center +0x200, Next
Gui, Add, Text, gViewLogs               x315 y228 w100 h23 BackgroundTrans cCCCCCC Border +Center +0x200, View Logs
Gui, Add, Text, gClearLogs              x415 y228 w100 h23 BackgroundTrans cCCCCCC Border +Center +0x200, Clear Logs
Gui, Add, Progress, x0 y250 w515 h1 0x10 BackgroundFFCC66 Disabled
...
Gui, Font, norm cCCCCCC s12 q5, Segoe UI Emoji
Gui, Add, Progress, x0 y430 w515 h1 0x10 BackgroundFFCC66 Disabled
Gui, Add, Text, vCurrentTrack x2 y432 w515 +Left +0x200, Now playing: (TestTrack)
Gui, Add, Progress, x0 y458 w515 h1 0x10 BackgroundFFCC66 Disabled
...
if (A_GuiEvent = "DoubleClick")
    ...
    Log("DEBUG", "Load playlist: " . sortCol . " written to: " . iniFile)
    Log("DEBUG", "Load playlist: " . sortDir . " written to: " . iniFile)
return
...
ConvertToMP3(filePath) {
    ...
    ffmpeg := A_ScriptDir . "\ffmpeg\ffmpeg.exe"
    if !FileExist(ffmpeg) {
        MsgBox, 16, Error, ffmpeg.exe not found in /ffmpeg
        Log("ERROR", "Convert to MP3, ffmpeg.exe not found in /ffmpeg")
        return
    }
...
ConfirmFileExists(file) {
    ...
    Gui, %guiID%:Font, s10 bold cRed q5
    Gui, %guiID%:Add, Text,, MP3 already exists:
    Gui, Font
    Gui, %guiID%:Font, s10 bold q5
...
global logFile
    ...
    if (needsRotation && FileExist( logfile)) {
        ...
        if (logSize > 1024000) {  ;>1MB
            ...
            FileMove, %logfile%, %A_ScriptDir%\rpcl3_music_player_%timestamp%.log
        }
        needsRotation := false
    }
